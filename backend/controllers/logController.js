...
export const getHeatmap = async (req, res) => {
  try {
    ...
    for (const log of logs) counts[log.completedDate] = ( counts[log.completedDate] || 0 ) + 1;
    const data = days.map((day) => ({ date: day, count: counts[day] || 0 }));
...
export const getHabitStats = async (req, res) => { 
  try {
    ...
    // monthly breakdwon ( last 6 months)
    const monthly = {};
    for (const l of logs) {
